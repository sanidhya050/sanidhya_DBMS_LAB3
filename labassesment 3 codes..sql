...
( select  t1.cus_id, t1.ord_amount, t1.cus_name from 
...
inner join 
(select product.pro_name, supplier_pricing_id from product 
inner join 
supplier_pricing
on 
supplier_pricing.pro_id=product.pro_id) as A1
on
`order`.pricing_id=A1.pricing_id where `order`.Cust_id = 2;
...
/* (6) Find the least expensive product from each categry and print the tble with category id , name , product name , ...alter
*/
...
inner join 
(select pro.id, min(supp_price) as Min_price from supplier_pricing group by pro_id)
